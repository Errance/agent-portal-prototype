...
`Dashboard` 顶部 KPI 与 `RevenueCenter / InvitePromotion / FriendsCenter` 页面的相同指标（累计注册、累计返佣、累计交易额）**必须由后端下发同一口径字段**，前端不再做跨页面独立聚合。

- `GET /agent/dashboard/kpi` 返回的 KPI 条目应与其它详情接口用**相同的统计窗口**（如 T+0、UTC+8 日历日）
- 若窗口不同，需要在 `DashboardKPI.note` 或字段上注明（如 `source_window: "today_utc8"`）
- 前端只做"展示 + 按筛选二次筛分"，不自行累加出一份"总佣金"与 Dashboard 对打

### 0.6 返佣比例边界（审计 M5，待产品确认）

当前前端 `validateRate` 按 `n >= cap` 拒绝相等（即子级比例必须**严格小于**代理商自身）。若产品允许"持平"（`n > cap` 才拒绝），需要：
1. 产品侧确认边界语义
2. 后端写接口与前端校验同步调整
