...
function aggregateSubAgents(list: SubAgent[]): SubAgentAggregate {
  let directUsdt = new Decimal(0),
    directUsdc = new Decimal(0)
  let rewardUsdt = new Decimal(0),
    rewardUsdc = new Decimal(0)
  for (const a of list) {
...
export default function FriendsCenter() {
  const {
    ...
    isFrozen,
    currentFlatFeeRate,
    currentProfitShareRate,
    currentEventRate,
  } = useAgent()
  const inviteesQ = useInvitees()
  const subAgentsQ = useSubAgents()
  const invitees = useMemo(() => inviteesQ.data ?? [], [inviteesQ.data])
  const subAgents = useMemo(() => subAgentsQ.data ?? [], [subAgentsQ.data])
  ...
  const inviteeColumns: Column<Invitee>[] = useMemo(
    () => [
        ...
        render: r => (
          <Box>
            ...
            <Text fontSize="12px" color="gray.200" mt="2px">
              {r.isSelf ? '代理商' : r.identityType === 'sub_agent' ? '子代理' : '普通用户'} ·{' '}
              {r.registeredAt.split(' ')[0]}
            </Text>
      ...
      },
      {
        ...
        render: r => (
          <Box>
            <Text color="text.100" title={remarks[r.uid] || r.remark || ''}>
              {truncateText(remarks[r.uid] || r.remark, 18)}
            </Text>
            {!r.isSelf && (
              <Box mt="6px">
                <PillButton
  ...
  const handleSaveRate = () => {
    const r = validateRate(
      { flatFee: editFF, profitShare: editPS, event: editEvent },
      {
    ...
    () => [
      {
        ...
        render: r => (
          <Box>
            ...
            <Text fontSize="12px" color="gray.200" mt="2px" title={r.nickname}>
              {truncateText(r.nickname, 16)} · {r.registeredAt.split(' ')[0]}
            </Text>
          </Box>
        ),
      },
      ...
      },
      {
        ...
        render: r => (
          <Box>
            <Text color="text.100" fontFamily="ISB" fontSize="14px">
              {fmtAmount(r.directCommUsdt, { style: 'thousand' })} USDT
            </Text>
            <Text color="text.100" fontFamily="ISB" fontSize="14px" mt="4px">
              {fmtAmount(r.directCommUsdc, { style: 'thousand' })} USDC
            </Text>
          </Box>
        ),
        sortable: true,
        sortKey: r => toNumber(r.directCommUsdt),
        minW: '160px',
      },
      {
        key: 'reward',
        ...
        render: r => (
          <Box>
            <Text color="text.100" fontFamily="ISB" fontSize="14px">
              {fmtAmount(r.platformRewardUsdt, { style: 'thousand' })} USDT
            </Text>
            <Text color="text.100" fontFamily="ISB" fontSize="14px" mt="4px">
              {fmtAmount(r.platformRewardUsdc, { style: 'thousand' })} USDC
            </Text>
          </Box>
        ),
        sortable: true,
        sortKey: r => toNumber(r.platformRewardUsdt),
        minW: '160px',
      },
      {
        key: 'action',
        ...
        render: r => (
          <Flex gap="8px" justify="flex-end">
            ...
            <PillButton variant="primary" to={`/revenue?source_uid=${r.uid}`}>
              贡献明细
            </PillButton>
          </Flex>
        ),
  ...
  return (
    <Box>
      <Tabs.Root value={tab} onValueChange={e => setTab(e.value)}>
        ...
        <Tabs.Content value="0">
          ...
          <DataTable
            ...
            error={
              inviteesQ.isError
                ? { message: toError(inviteesQ.error).message, retry: () => inviteesQ.refetch() }
                : null
            }
          />
        </Tabs.Content>
        ...
        <Tabs.Content value="1">
          ...
          <DataTable
            ...
            error={
              subAgentsQ.isError
                ? { message: toError(subAgentsQ.error).message, retry: () => subAgentsQ.refetch() }
                : null
            }
          />
        </Tabs.Content>
      ...
      <ModalShell open={!!editUid} onClose={() => setEditUid(null)} width="440px">
        ...
        <Text fontSize="13px" color="gray.100" mb="8px">
          UID: {editUid}
        </Text>
        <ChakraInput
          w="100%"
        ...
        <Flex justify="flex-end" gap="12px" mt="32px">
          <PillButton variant="ghost" size="lg" shape="rect" onClick={() => setEditUid(null)}>
            取消
          </PillButton>
          <PillButton
            variant="solid"
      ...
      <ModalShell open={!!editRateAgent} onClose={() => setEditRateAgent(null)} width="480px">
        ...
        <Flex direction="column" gap="20px">
          ...
          <RateFormInput
            ...
            step="0.01"
            error={rateErrors.event}
            extra={`上限: ${currentEventRate}%`}
          />
        </Flex>
        <Flex justify="flex-end" gap="12px" mt="32px">
          <PillButton variant="ghost" size="lg" shape="rect" onClick={() => setEditRateAgent(null)}>
            取消
          </PillButton>
          <PillButton variant="solid" size="lg" shape="rect" onClick={handleSaveRate}>
            保存
          </PillButton>
        </Flex>
      </ModalShell>
