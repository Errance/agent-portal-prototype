...
export default function InvitePromotion() {
  ...
  const handleRevoke = () => {
    ...
    setCodes(prev =>
      prev.map(c => (c.code === revokeCode.code ? { ...c, status: 'revoked' as const } : c)),
    )
    setRevokeCode(null)
  }
      ...
      },
      {
        ...
        render: r => (
          <Text color="text.100" fontFamily="ISB">
            {r.subFlatFeeRate.toFixed(2)}%
          </Text>
        ),
        sortable: true,
        sortKey: r => r.subFlatFeeRate,
      },
      {
        key: 'ps',
        label: 'PS 下级比例',
        ...
        render: r => (
          <Text color="text.100" fontFamily="ISB">
            {r.subProfitShareRate.toFixed(4)}%
          </Text>
        ),
        sortable: true,
        sortKey: r => r.subProfitShareRate,
      },
      {
        key: 'ev',
        label: '事件下级比例',
        ...
        render: r => (
          <Text color="text.100" fontFamily="ISB">
            {r.subEventRate.toFixed(2)}%
          </Text>
        ),
        sortable: true,
        sortKey: r => r.subEventRate,
      },
      {
        key: 'regs',
        label: '注册',
          ...
          <Flex gap="8px" justify="flex-end">
            ...
            {r.status === 'active' && (
              <>
                ...
                <PillButton variant="danger" disabled={isFrozen} onClick={() => setRevokeCode(r)}>
                  作废
                </PillButton>
              </>
            )}
    ...
    <Box>
        ...
        <Flex align="center" gap="32px">
          ...
          <Flex gap="24px">
            <Box>
              ...
              <Text fontFamily="ISB" fontSize="18px" color="theme">
                {currentFlatFeeRate.toFixed(2)}%
              </Text>
            </Box>
            <Box>
              <Text
              ...
              <Text fontFamily="ISB" fontSize="18px" color="theme">
                {currentProfitShareRate.toFixed(4)}%
              </Text>
            </Box>
            <Box>
              <Text
      ...
      >
        <FilterItem label="推广码">
          <Input value={fCode} onChange={setFCode} placeholder="精确搜索" />
        </FilterItem>
        <FilterItem label="状态">
          <Select
            value={statusFilter}
      ...
      <DataTable
        ...
        error={
          codesQ.isError
            ? { message: toError(codesQ.error).message, retry: () => codesQ.refetch() }
            : null
        }
      />

      <ModalShell open={showCreate} onClose={() => setShowCreate(false)} width="480px">
        <Text fontFamily="ISB" fontSize="20px" mb="24px" color="text.100" letterSpacing="-0.5px">
          新建推广码
        </Text>
        <Flex direction="column" gap="20px">
          <RateFormInput
            label="Flat Fee 下级返佣比例（%）"
            value={ffRate}
            onChange={setFfRate}
            step="0.01"
            error={errors.ff}
            extra={`上限: ${currentFlatFeeRate}%`}
          />
          <RateFormInput
            label="Profit Share 下级返佣比例（%）"
            value={psRate}
            onChange={setPsRate}
            step="0.0001"
            error={errors.ps}
            extra={`上限: ${currentProfitShareRate}%`}
          />
          <RateFormInput
            label="事件合约下级返佣比例（%）"
            value={eventRate}
        ...
        <Flex justify="flex-end" gap="12px" mt="32px">
          <PillButton variant="ghost" size="lg" shape="rect" onClick={() => setShowCreate(false)}>
            取消
          </PillButton>
          <PillButton variant="solid" size="lg" shape="rect" onClick={handleCreate}>
            创建
          </PillButton>
        </Flex>
      </ModalShell>
      ...
      <ModalShell open={!!editCode} onClose={() => setEditCode(null)} width="480px">
        <Text fontFamily="ISB" fontSize="20px" mb="8px" color="text.100" letterSpacing="-0.5px">
          编辑推广码
        </Text>
        {editCode && (
        ...
        <Flex direction="column" gap="20px">
          ...
          <RateFormInput
            ...
            step="0.01"
            error={editErrors.event}
            extra={`上限: ${currentEventRate}%`}
          />
        </Flex>
        <Text fontSize="13px" color="gray.200" mt="24px">
          修改不追溯，仅影响后续通过该推广码注册的新用户。
        </Text>
        <Flex justify="flex-end" gap="12px" mt="32px">
          <PillButton variant="ghost" size="lg" shape="rect" onClick={() => setEditCode(null)}>
            取消
          </PillButton>
          <PillButton variant="solid" size="lg" shape="rect" onClick={handleEdit}>
            保存
          </PillButton>
        </Flex>
      </ModalShell>
      ...
      <ModalShell open={!!revokeCode} onClose={() => setRevokeCode(null)} width="440px">
        <Text fontFamily="ISB" fontSize="20px" mb="16px" color="text.100" letterSpacing="-0.5px">
          确认作废推广码
        </Text>
        {revokeCode && (
          <Text fontSize="14px" color="gray.100" mb="24px" lineHeight="1.6">
            确定要作废推广码{' '}
            <Text as="span" fontFamily="ISB" color="text.100">
              {revokeCode.code}
            </Text>{' '}
            吗？
            <br />
        ...
        <Flex justify="flex-end" gap="12px">
          <PillButton variant="ghost" size="lg" shape="rect" onClick={() => setRevokeCode(null)}>
            取消
          </PillButton>
          <PillButton variant="dangerSolid" size="lg" shape="rect" onClick={handleRevoke}>
            确认作废
          </PillButton>
        </Flex>
      </ModalShell>
      ...
      >
        <Flex justify="space-between" align="center" mb="24px">
          <Text fontFamily="ISB" fontSize="20px" color="theme" letterSpacing="-0.5px">
            推广码创建成功
          </Text>
          <Box
            as="button"
        ...
        {createdCode && (
          <Flex direction="column" gap="24px">
            <Box>
              ...
              <Text fontFamily="ISB" fontSize="32px" color="text.100" lineHeight="1">
                {createdCode.code}
              </Text>
            </Box>
            <Flex gap="32px">
              <Box>
                ...
                <Text fontFamily="ISB" fontSize="16px" color="text.100">
                  {createdCode.subFlatFeeRate.toFixed(2)}%
                </Text>
              </Box>
              <Box>
                <Text
                ...
                <Text fontFamily="ISB" fontSize="16px" color="text.100">
                  {createdCode.subProfitShareRate.toFixed(4)}%
                </Text>
              </Box>
              <Box>
                <Text
                ...
                <Text fontFamily="ISB" fontSize="16px" color="text.100">
                  {createdCode.subEventRate.toFixed(2)}%
                </Text>
              </Box>
            </Flex>
            <Box>
