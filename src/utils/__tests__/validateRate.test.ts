...
describe('validateRate', () => {
  ...
  it('0 或负数 -> 必须大于 0', () => {
    ...
    if (!zero.ok) expect(zero.errors.ff).toBe('必须大于 0')
    expect(neg.ok).toBe(false)
    if (!neg.ok) expect(neg.errors.ff).toBe('必须大于 0')
  })
  ...
  it('等于或超过上限 -> 报错', () => {
    ...
    if (!eq.ok) expect(eq.errors.ff).toBe('必须 < 1.5%')
    expect(over.ok).toBe(false)
    if (!over.ok) expect(over.errors.ff).toBe('必须 < 1.5%')
  })
